...
class TestOriginFareResult:
    ...
    def test_rtw_fares(self, osl_result):
        assert len(osl_result.rtw_fares) == 6

    def test_done_fares(self, osl_result):
        done = osl_result.done_fares
        assert len(done) == 2
        assert done[0].fare_basis == "DONE3"
        assert done[1].fare_basis == "DONE4"

    def test_aone_fares(self, osl_result):
        aone = osl_result.aone_fares
        assert len(aone) == 2
        assert aone[0].fare_basis == "AONE3"

    def test_lone_fares(self, osl_result):
        lone = osl_result.lone_fares
        assert len(lone) == 2
        assert lone[0].fare_basis == "LONE3"

    def test_get_fare(self, osl_result):
...
class TestFareComparisonResult:
    ...
    def test_ranking_for(self, comparison):
        ...
        assert ranking[0] == ("BOM", pytest.approx(4800.00))
        assert ranking[1] == ("NRT", pytest.approx(5200.00))
        assert ranking[2] == ("OSL", pytest.approx(5957.88))

    def test_cheapest_for_missing(self, comparison):
        assert comparison.cheapest_for("AONE6") is None

    def test_ranking_for_missing(self, comparison):
        assert comparison.ranking_for("AONE6") == []
...
class TestExpertFlyerFareScraper:
    ...
    def test_build_fare_url(self):
        ...
        assert "destination=OSL" in url  # RTW: same origin/dest
